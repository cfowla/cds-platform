...
def test_rule_contract_exposes_only_the_minimal_evaluate_method() -> None:
    public_names = {name for name in vars(RenalDoseRule) if not name.startswith("_")}

    assert public_names == {"evaluate"}


def test_evaluate_signature_uses_context_renal_result_content_and_structured_result() -> None:
    ...
    assert parameters[3].kind is Parameter.POSITIONAL_ONLY
    assert type_hints["context"] is RenalDoseEvaluationContext
    assert type_hints["renal_function"] is RenalFunctionResult
