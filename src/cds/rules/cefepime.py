...
CEFEPIME_RULE_IMPLEMENTATION_VERSION = "1.0.0"
...
def evaluate_cefepime_rule(
    *,
    ...
    round the renal value, or infer missing context.
    """
    ...
    if order.medication.code != content.medication.id:
        return _incomplete(
            base_result,
            "Medication identifier does not exactly match cefepime content.",
        )

    if regimen_id != content.regimen.id:
        return _incomplete(
            base_result,
            "Regimen identifier does not exactly match cefepime content.",
        )

    if order.indication.code not in content.regimen.indication_ids:
        return _incomplete(
            base_result,
            "Indication identifier is missing or unsupported for this regimen.",
        )

    if order.route.code != content.regimen.route_id:
        return _incomplete(base_result, "Route identifier does not exactly match cefepime content.")

    if formulation_id != content.regimen.formulation_id:
        return _incomplete(
            base_result,
            "Formulation identifier does not exactly match cefepime content.",
        )

    if not _quantity_matches(order.dose, content.regimen.base_dose):
        return _incomplete(
            base_result,
            "Dose value or unit does not exactly match cefepime content.",
        )

    if not _quantity_matches(order.frequency_interval, content.regimen.frequency_interval):
        return _incomplete(
            base_result,
            "Frequency interval does not exactly match cefepime content.",
        )

    if not _quantity_matches(order.infusion_duration, content.regimen.infusion_duration):
        return _incomplete(
            base_result,
            "Infusion duration does not exactly match cefepime content.",
        )

    renal_value = renal_function.value.value
    supported_context = content.supported_context
    if (
        renal_function.age_years is None
        or renal_function.age_years < supported_context.minimum_age_years
    ):
        return _incomplete(
            base_result,
            "Age is missing or outside the reviewed adult content context.",
        )

    if renal_function.method.value != supported_context.renal_method:
        return _incomplete(base_result, "Renal method does not exactly match cefepime content.")

    if renal_value is None or renal_function.value.unit != supported_context.renal_unit:
        return _incomplete(
            base_result,
            "Renal value or unit is missing or does not exactly match content.",
        )

    if renal_function.normalized_to_bsa is not False:
        return _incomplete(base_result, "An explicitly unindexed renal result is required.")

    if renal_function_stable is not supported_context.renal_function_stable:
        return _incomplete(base_result, "Renal-stability context does not exactly match content.")

    if renal_replacement_therapy is not supported_context.renal_replacement_therapy:
        return _incomplete(
            base_result,
            "Renal-replacement-therapy context does not exactly match content.",
        )
    ...
    if not renal_band_matches(
        ...
        return _incomplete(base_result, "Renal value is outside the content renal domain.")
...
def _no_recommendation_result(result: RuleResult, band: RenalDoseBandContent) -> RuleResult:
    ...
    result.summary = (
        ...
        or "Matched content permits no dose recommendation."
    )
